...
def user_id = JC.internal.param('user_id');
...
AuthUser current_user = AuthUserService.INSTANCE().get_by_id(user_id, pid)
if (current_user == null) {
	return new ArrayList<AppFunction>();
}
AuthRoleService role_service = AuthRoleService.INSTANCE();
List<AuthRole> user_roles = role_service.user_roles(current_user, pid);

List<String> role_mods = PmodMMService.INSTANCE().find_roles_functions(app_code, user_roles);
FuncHolder fh_ser = FuncHolder.INSTANCE;
List<AppFunction> functions = fh_ser.get_appcode_funcs_by_ids(app_code, role_mods);
